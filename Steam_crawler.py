...
def GetDataFromGame(game_url):
    ...
    game_developer = game_soup.find('div',attrs={'class':'summary column'}).find('a').string.strip()
    game_genre = []
    game_genre_prep = game_soup.find('span',attrs={'data-panel':'{"flow-children":"row"}'}).find_all('a',string)
...
if __name__ == '__main__':
    baseurl = "https://store.steampowered.com/search/results/?&sort_by=Reviews_DESC&snr=1_7_7_7000_7&filter=topsellers&infinite=1"
    ...
    for game_url in links:
        curr_game_name, curr_game_date, curr_game_developer, curr_game_genre, curr_game_review_count, curr_game_review_positive, curr_game_review_negative, curr_game_price = GetDataFromGame(
            game_url)
